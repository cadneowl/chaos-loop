...
_PERCENTILE_FIELDS = frozenset({"mean", "p50", "p95", "p99"})


class MetricDirection(StrEnum):
    """Which way is *worse* for a metric, so the comparison knows a regression."""

    HIGHER_WORSE = "higher_worse"  # latency, error rate — the default
    LOWER_WORSE = "lower_worse"  # throughput, availability, success rate


class MetricThreshold(BaseModel):
    """One metric's drift budget: how much its percentile may move before it counts.

    Drives both axes — the acute (under-fault vs clean baseline) verdict and the
    chronic (clean baseline vs golden) drift — off the same tolerance.
    """

    metric: str
    query: str = Field(default="", description="PromQL to sample this metric (oracle config).")
    percentile: Literal["mean", "p50", "p95", "p99"] = "p95"
    # A regression is a move of more than ``max_ratio`` in the *worse* direction:
    # for HIGHER_WORSE, current > golden * max_ratio; for LOWER_WORSE,
    # current < golden / max_ratio. 1.10 = tolerate a 10% drift.
    max_ratio: float = Field(default=1.10, gt=1.0)
    direction: MetricDirection = MetricDirection.HIGHER_WORSE
    # Ignore regressions when the golden percentile is below this floor — avoids
    # flapping on near-zero baselines where a tiny absolute move is a huge ratio.
    abs_floor: float = Field(default=0.0, ge=0.0)
...
class MetricDrift(BaseModel):
    """One metric's chronic drift: a golden percentile vs the current baseline's.

    ``regressed`` is True when the current value moved past the golden by more
    than the threshold's ``max_ratio`` in the metric's *worse* direction.
    """
    ...
    max_ratio: float
    regressed: bool
    missing: bool = False  # metric present in the golden but not sampled this run (or vice-versa)

    @computed_field  # type: ignore[prop-decorator]
    @property
    def ratio(self) -> float | None:
        """Current / golden (the raw move). ``None`` when not comparable."""
        if self.missing or self.golden_value == 0:
            return None
        return self.current_value / self.golden_value
...
class ScenarioDrift(BaseModel):
    ...
    metric_drifts: list[MetricDrift] = Field(
        default_factory=list,
        description="Per-metric chronic drift of the clean baseline vs the golden's distribution.",
    )
    ...
    @computed_field  # type: ignore[prop-decorator]
    @property
    def metric_regressions(self) -> list[str]:
        return [m.metric for m in self.metric_drifts if m.regressed]
    ...
    def drifted(self) -> bool:
        if self.unassessed:
            return False
        return bool(self.regressed) or any(m.regressed for m in self.metric_drifts)
...
class DriftReport(BaseModel):
    ...
    def regressed_scenarios(self) -> int:
        """Scenarios that drifted — a baseline journey regression OR a metric one."""
        return sum(1 for s in self.scenarios if s.drifted)
